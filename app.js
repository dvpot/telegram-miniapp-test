  ...
  try {
    ...
    try {
      ...
      if (voices && voices.length) {
        const v = voices.find(x => x.lang?.toLowerCase().startsWith(langTag.split('-')[0].toLowerCase()));
        if (v) utter.voice = v;
      }
    } catch (voiceErr) {
      console.warn('Ошибка голосов:', voiceErr);
    }
    ...
    try {
      ...
      console.warn('Ошибка озвучки:', speakErr);
    }

  } catch (e) {
    console.warn('Ошибка в функции speak:', e);
  }
}
...
speakENBtn.addEventListener('click', (e) => {
  ...
  try {
    if (speakENBtn.classList.contains('visible')) {
      speak(current.word, 'en-US');
    }
  } catch (err) {
    console.warn('Ошибка EN TTS:', err);
  }
});
speakRUBtn.addEventListener('click', (e) => {
  ...
  try {
    ...
    console.warn('Ошибка RU TTS:', err);
  }
});
...
(async function init(){
  try {
    ...
    console.warn('Ошибка инициализации голосов:', initErr);
  }
